...
[[ssec_requiredlibs,Required Software Libraries]]
...
IMPORTANT: When compiling LIS with HDF 4 support, you must also download and compile HDF-EOS2 version 2.19v1.00 or higher from http://hdfeos.org/software/library.php.
...
The following tables provide some specific examples of the compiler and library versions that LIS has been tested with.  See Appendix <<More specific examples>> for more examples.
...
| `LIS_CPPC`      | {cpp} compiler                   | required
