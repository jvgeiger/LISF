...
!BOP
!
! !ROUTINE: timeinterp_metsim
!  \label{timeinterp_metsim}
! 
! !REVISION HISTORY: 
! 26 Jan 2007: Hiroko Kato; Initial version of code adapted from LIS
! 25 Jun 2007: Hiroko Kato; upgraded for LISv5.0
! 30 Apr 2020: Zhuo Wang: updated for MetSim forcing data 
! 
! !INTERFACE:
subroutine timeinterp_metsim(n, findex)
! !USES:
  use ESMF
...
!EOP
  ...
  btime=metsim_struc(n)%metsimtime1
  call LIS_time2date(btime,bdoy,gmt1,byr,bmo,bda,bhr,bmn)
  btime=metsim_struc(n)%metsimtime2
  call LIS_time2date(btime,bdoy,gmt2,byr,bmo,bda,bhr,bmn)
 
 !-----------------------------------------------------------------------
 !  Interpolate Data in Time
 !-----------------------------------------------------------------------
  wt1=(metsim_struc(n)%metsimtime2-LIS_rc%time)/&
       (metsim_struc(n)%metsimtime2-metsim_struc(n)%metsimtime1)
  wt2=1.0-wt1
...
! JVG: Disable zterp for testing
   ...
   do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       ...
       if (metsim_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then
          swd(t) = zw1 * metsim_struc(n)%metdata2(kk, 3, index1)
       endif
      
       if (swd(t) < 0) then
          write(LIS_logunit,*) '[ERR] MetSim -- 2 warning!!! SW radiation is negative!!'
          write(LIS_logunit,*) 'sw=', swd(t), '... negative'
          write(LIS_logunit,*) 'MetSim =', metsim_struc(n)%metdata2(kk,3,index1)
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if (metsim_struc(n)%metdata2(kk,1,index1).ne.LIS_rc%udef) then
           tmp(t) = metsim_struc(n)%metdata2(kk,1,index1)
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if (metsim_struc(n)%metdata2(kk,2,index1).ne.LIS_rc%udef) then
          q2(t) = metsim_struc(n)%metdata2(kk,2,index1)
        endif
     enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if (metsim_struc(n)%metdata2(kk,4,index1).ne.LIS_rc%udef) then
          lwd(t) = metsim_struc(n)%metdata2(kk,4,index1)
        endif
      enddo
  enddo
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        if (metsim_struc(n)%metdata2(kk,5,index1).ne.LIS_rc%udef) then
          uwind(t) = metsim_struc(n)%metdata2(kk,5,index1)
        endif
     enddo
  enddo
...
end subroutine timeinterp_metsim
