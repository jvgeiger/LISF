...
!              vlevel=1, unit="m", direction="-",            &
         ...
         ! Note: LIS_MOC_TOTALSOILWAT is the sum of multi-layer soil moisture (1-layer)
...
!              var(iLookTYPE%hruId)
          ...
          nlayer_real = nSnowData(iGRU) + nSoilData(iGRU) 
...
!         do l=1, 13
          do l=1, 8
            ...
            if(l .le. nlayer_real) then
             dat2 = summa1_struc(n)%progStruct%gru(iGRU)%hru(iHRU)%var(iLookPROG%mLayerTemp)%dat(l)
           endif   ! l < nlayer_real
           call LIS_diagnoseSurfaceOutputVar(n, iGRU, LIS_MOC_MLAYERTEMP, value = dat2,  &
...
!          do l=1, 8
           do l=1, 3
             dat2 = summa1_struc(n)%progStruct%gru(iGRU)%hru(iHRU)%var(iLookPROG%mLayerMatricHead)%dat(l)
             call LIS_diagnoseSurfaceOutputVar(n, iGRU, LIS_MOC_MLAYERMOIST, value = dat2,  &
                  vlevel=l, unit="m", direction="-", surface_type = LIS_rc%lsm_index)
            enddo   ! l, layers
...
!           do l=1, 13
            do l=1, 8
               dat2=0
               if(l .le. nlayer_real) then
                 dat2 = summa1_struc(n)%progStruct%gru(iGRU)%hru(iHRU)%var(iLookPROG%mLayerVolFracWat)%dat(l)
               endif   ! l < nlayer_real
               call LIS_diagnoseSurfaceOutputVar(n, iGRU, LIS_MOC_MLAYERVOLFRACWAT, value = dat2,  &
