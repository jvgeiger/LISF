...
! !DESCRIPTION:
!
!  Forcing-only option (summa2) for calling the forcing transfer routines.
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]
...
!EOP
   ...
   call ESMF_TimeSet(referenceTime, yy = 1990, &
      ...
      rc = status)
   call LIS_verify(status,'error in ESMF_TimeSet:referenceTime in summa2_f2t')
   ...
   call ESMF_TimeSet(currentTime, yy = LIS_rc%yr, &
      ...
      calendar = LIS_calendar, &
      rc = status)
   call LIS_verify(status,'error in ESMF_TimeSet:currentTime in summa2_f2t')
