...
|CDL |USDA NASS Cropland Data Layer (CDL). Currently LDT uses a reprocessed version with 0,01 degree grid spacing.  
...
|CDL |256 classed; Used by USDA NASS
|===
...
`CDL start year:` indicates the first year of CDL data to be processed. CDL data vary annually. This option is only used if CDL crop classification is used.
...
`CDL end year:` indicates the last year of CDL data to be processed. CDL data vary annually. This option is only used if CDL crop classification is used.
